...
class Hospital extends React.Component {
  ...
  render() {
    let {hospital, service, serviceIcon, itemCount} = this.props;
    return (<View style={styles.info_blok}>
      ...
      <Line/>
      <View style={{flexDirection: 'row', alignItems: 'center', flex: 1.5}}>
        <Image style={{flex:0.08, height:20, resizeMode: 'contain', marginRight: 5}}
               source={serviceIcon}/>
        <Text style={{flex:1}}>{service.name}</Text>
        <Text style={{fontSize: FontSize.small, color: 'black'}}>¥ {service.servicePrice} </Text>
        <Text style={{fontSize: FontSize.small}}>x {itemCount}</Text>
      </View>
    </View>);
  }
}
  ...
  render() {
    ...
    return (<View
      ...
      <View style={{flex:1, flexDirection: 'row', padding: 5}}>
        <Text style={{flex:1, color: '#9b9b9b'}}>奖励金</Text>
        <Text>¥ {service.serviceDiscount}</Text>
      </View>
      <View style={{flex:1, flexDirection: 'row', padding: 5}}>
        <Text style={{flex:1, color: '#9b9b9b'}}>总额</Text>
...
class TimeInfo extends React.Component {
  render() {
    ...
    return (<View style={{flex:0.25, flexDirection: 'column', marginHorizontal:10, marginBottom: 5}}>
