...
export default class CaseDetail extends React.Component {
  ...
  render() {
    let {caseBook} = this.props.navigation.state.params;
    return (<ScrollView style={{flex:1, backgroundColor: colors.bkColor}}>
      <View style={{backgroundColor: 'white', margin: 10}}>
        <View style={{backgroundColor: 'white', flexDirection: 'row', paddingVertical: 10}}>
          <Text style={{fontSize: FontSize.large, marginLeft: 10}}>{caseBook.name}</Text>
          <View style={{flex:1}}/>
          <Text style={{alignSelf: 'flex-end', marginRight: 10, color: colors.labelColor}}>编辑</Text>
        </View>
        <Text style={{fontSize: FontSize.small, marginLeft:10, marginBottom:10}}>基本信息</Text>
      ...
      <View style={{backgroundColor: 'white', margin: 10, alignItems: 'center'}}>
        ...
        <Text style={{color: colors.bkColor}}>在当前病例添加一条记录</Text>
      </View>
