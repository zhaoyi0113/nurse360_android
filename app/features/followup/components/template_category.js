...
export default class TemplateCategory extends React.Component {
  ...
  render() {
    let {templateCategoryItems, navigation, patient, loadReadFollowUpList} = this.props;
        ...
        templateCategoryItems.map((template, i) => {
          return (<View key={i} style={{height:80}}>
            <CommonRowCell title={template.title}
                           ...
                           onClick={()=>navigation.navigate('TemplateDetail', {template:template, submit:true, patient, loadReadFollowUpList})}
