...
export default class NewFollowUp extends React.Component {
  ...
  render() {
    const {templateList, navigation, patient, loadReadFollowUpList, loadUnreadFollowUpList} = this.props;
    return (<View style={{backgroundColor: colors.bkColor, flex:1}}>
      ...
      <TouchableHighlight underlayColor={colors.underlayColor}
                          onPress={()=> navigation.navigate('NewQuestionFollowUp', {patient, loadReadFollowUpList})}>
        ...
        templateList.map((template, i) => {
          return <View key={i} style={{height: 80}}>
            <CommonRowCell title={template.name}
                           ...
                           onClick={()=>navigation.navigate('TemplateCategory', {categoryId: template.id, patient: patient, loadReadFollowUpList})}
